...
class MovieImportRepository:
    ...
    async def exists_movie_import_by_title(self, title: str) -> bool:
        ...
        """
        res = await self.session.execute(select(exists().where(MovieImport.title == title)))
        return res.scalar()


    async def get_movie_import_or_none_by_id(self, movie_import_id: int) -> MovieImport | None:
        """
        Get movie by id or return None.
        :param movie_id:
        :return: Movie object or None
        """
        res = await self.session.execute(select(MovieImport).filter(MovieImport.id == movie_import_id))
    ...
    async def get_movie_or_none_by_title(self, title: str) -> MovieImport | None:
        ...
        :return: Movie object or None
        """
        res = await self.session.execute(select(MovieImport).filter(MovieImport.title == title))
        return res.scalar_one_or_none()
